...
def separate_by_event_name(df_remainer, df_combined=None, key=None, meta_key=None, type_name='end'):
    ...
    if type_name == 'end':
        df_subset = df_remainer[df_remainer.event_name.apply(lambda x: x[-len(key):] == key)].copy()
        df_remainer_new = df_remainer[df_remainer.event_name.apply(lambda x: x[-len(key):] != key)].copy()
    else:
        df_subset = df_remainer[df_remainer.event_name.apply(lambda x: x[:len(key)] == key)].copy()
        df_remainer_new = df_remainer[df_remainer.event_name.apply(lambda x: x[:len(key)] != key)].copy()
...
#     print(f'Type "{key}". Number of events: {events_found:,.0f} \n')
    if events_found == 0:
        return df_remainer_new, df_combined
    ...
    if df_combined is None:
        ...
        df_combined = pd.concat([df_combined, df_subset], sort=False)
